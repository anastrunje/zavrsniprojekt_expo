...
export default class Board extends React.Component {
  ...
  checkErrors = ({ grid }) => {
    ...
    for (let i = 0; i < width * height; i++) {
      this.checkDuplicates(
        this.getRegion({ grid, regionCol: null, regionRow: i })
      );
    }

    for (let j = 0; j < width * height; j++) {
      this.checkDuplicates(
        this.getRegion({ grid, regionCol: j, regionRow: null })
      );
    }
  ...
  errorStyle = (cell) => {
    return cell.error
      ? { backgroundColor: `rgba(255, 116, 102, ${cell.error / 10})` }
      : {};
  };
  ...
  lockedStyle = (cell) => {
    ...
    else if (cell.locked) return { backgroundColor: "#ccc" };
    else return {};
  };
  ...
  renderCellText = ({ duplicate, locked, value }) => {
    ...
    if (duplicate) {
      ...
      return <Text style={cellText}>{displayValue}</Text>;
    }
  };
  ...
  renderCells({ cell, rowIdx, colIdx }) {
    return (
      <TouchableOpacity
        style={[{ flex: 1 }, this.borderStyle(rowIdx, colIdx)]}
  ...
  selectorPressed = (selectedNumber) => {
    ...
    if (selected !== null) {
      ...
      if (
        !this.countItems({ grid, prop: "value", value: 0 }) &&
        this.countItems({ grid, prop: "error", value: 0 }) ===
      ...
      <Animatable.View animation="fadeInUp" duration={2000}>
        <View style={boardContainer}>
          {this.state.grid.map((row, rowIdx) =>
            this.renderRow({ row, rowIdx })
          )}
        </View>
        <Animatable.View animation="lightSpeedIn" delay={1000}>
          <Selector
            onPress={(selectedNumber) => this.selectorPressed(selectedNumber)}
          />
        </Animatable.View>
      </Animatable.View>
    );
